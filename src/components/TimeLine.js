  ...
  render() {
    return (
      <List
        ...
        style={[styles.listview, this.props.style]} onLayout={({ nativeEvent: { layout: { height } } }) => {
        this.props.updateHeight(height)
      }}>
        {this.state.data.map((rowData, i) => (
          <View key={i} style={[styles.rowContainer, this.props.rowContainerStyle]}>
            {this.renderTime(rowData, i, i)}
            {this.renderEvent(rowData, i, i)}
            {this.renderCircle(rowData, i, i)}
          </View>
        ))}
      </List>
    );
